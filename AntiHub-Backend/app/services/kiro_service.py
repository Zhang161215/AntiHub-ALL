...
class KiroService:
    ...
    async def _get_user_plugin_key(self, user_id: int) -> str:
        ...
        try:
            await self.redis.set(
                cache_key, decrypted_key, expire=PLUGIN_API_KEY_CACHE_TTL
            )
            logger.debug(
                f"plugin_api_key 已缓存 (kiro): user_id={user_id}, ttl={PLUGIN_API_KEY_CACHE_TTL}s"
            )
        except Exception as e:
    ...
    async def get_oauth_authorize_url(
        self, user_id: int, provider: str, is_shared: int = 0
    ) -> Dict[str, Any]:
        ...
        return await self._proxy_request(
            ...
            json_data={"provider": provider, "is_shared": is_shared},
        )

    async def get_oauth_status(self, user_id: int, state: str) -> Dict[str, Any]:
        """轮询Kiro OAuth授权状态（通过插件API）"""
        return await self._proxy_request(
            user_id=user_id, method="GET", path=f"/api/kiro/oauth/status/{state}"
        )
    ...
    async def create_account(
        self, user_id: int, account_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """创建Kiro账号（通过插件API）"""
        return await self._proxy_request(
            user_id=user_id,
    ...
    async def get_accounts(self, user_id: int) -> Dict[str, Any]:
        """获取Kiro账号列表（通过插件API）"""
        return await self._proxy_request(
            user_id=user_id, method="GET", path="/api/kiro/accounts"
        )

    async def get_account(self, user_id: int, account_id: str) -> Dict[str, Any]:
        """获取单个Kiro账号（通过插件API）"""
        return await self._proxy_request(
            user_id=user_id, method="GET", path=f"/api/kiro/accounts/{account_id}"
        )
    ...
    async def update_account_status(
        self, user_id: int, account_id: str, status: int
    ) -> Dict[str, Any]:
        """更新Kiro账号状态（通过插件API）"""
        return await self._proxy_request(
            user_id=user_id,
    ...
    async def update_account_name(
        self, user_id: int, account_id: str, account_name: str
    ) -> Dict[str, Any]:
        """更新Kiro账号名称（通过插件API）"""
        return await self._proxy_request(
            user_id=user_id,
    ...
    async def delete_account(self, user_id: int, account_id: str) -> Dict[str, Any]:
        """删除Kiro账号（通过插件API）"""
        return await self._proxy_request(
            user_id=user_id, method="DELETE", path=f"/api/kiro/accounts/{account_id}"
        )
    ...
    async def get_model_mappings(self) -> Dict[str, Any]:
        """获取Kiro模型映射表（透传插件API）"""
        return await self._proxy_admin_request(
            method="GET",
            path="/api/kiro/model-mappings",
        )

    async def update_model_mappings(self, mappings: Dict[str, str]) -> Dict[str, Any]:
        """批量更新Kiro模型映射表（透传插件API）"""
        return await self._proxy_admin_request(
            method="PUT",
            path="/api/kiro/model-mappings",
    ...
    async def add_model_mapping(
        self, frontend_model: str, kiro_model: str
    ) -> Dict[str, Any]:
        """添加或更新单个模型映射（透传插件API）"""
        return await self._proxy_admin_request(
            method="POST",
            path="/api/kiro/model-mappings",
            json_data={"frontend_model": frontend_model, "kiro_model": kiro_model},
        )

    async def delete_model_mapping(self, frontend_model: str) -> Dict[str, Any]:
        """删除单个模型映射（透传插件API）"""
        return await self._proxy_admin_request(
            method="DELETE",
            path=f"/api/kiro/model-mappings/{frontend_model}",
        )

    async def reset_model_mappings(self) -> Dict[str, Any]:
        """重置模型映射为默认值（透传插件API）"""
        return await self._proxy_admin_request(
            method="POST",
            path="/api/kiro/model-mappings/reset",
        )

    async def get_models(self, user_id: int) -> Dict[str, Any]:
        """获取Kiro模型列表（通过插件API）"""
        return await self._proxy_request(
            user_id=user_id, method="GET", path="/v1/kiro/models"
        )
    ...
    async def chat_completions_stream(self, user_id: int, request_data: Dict[str, Any]):
        """Kiro聊天补全（流式，通过插件API）"""
