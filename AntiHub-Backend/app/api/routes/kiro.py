...
async def submit_oauth_callback(
    request: KiroOAuthCallbackRequest, service: KiroService = Depends(get_kiro_service)
):
    """
    ...
    """
    try:
        if not request.callback_url or not request.callback_url.lower().startswith(
            "kiro://"
        ):
...
async def get_oauth_status(
    ...
    """
    try:
        result = await service.get_oauth_status(user_id=current_user.id, state=state)
        return result
    except UpstreamAPIError as e:
        return JSONResponse(
            status_code=e.status_code,
...
@router.post("/accounts", summary="创建Kiro账号", description="创建新的Kiro账号")
async def create_account(
    ...
    try:
        ...
        if auth_method == "IdC" and (
            not account_data.get("client_id") or not account_data.get("client_secret")
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
@router.get(
    "/accounts", summary="获取Kiro账号列表", description="获取当前用户的所有Kiro账号"
)
...
async def get_account(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"账号不存在: {str(e)}"
        )
...
async def update_account_status(
    ...
    try:
        ...
        if status_value not in [0, 1]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="status必须是0或1"
            )

        result = await service.update_account_status(
            current_user.id, account_id, status_value
        )
        return result
...
async def update_account_name(
    ...
    try:
        ...
        if not account_name:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="account_name不能为空"
            )

        result = await service.update_account_name(
            current_user.id, account_id, account_name
        )
        return result
    except HTTPException:
        raise
...
@router.delete(
    "/accounts/{account_id}", summary="删除Kiro账号", description="删除指定的Kiro账号"
)
...
@router.get(
    "/model-mappings",
    summary="获取模型映射表",
    description="获取 Kiro 前端模型名到真实模型名的映射关系",
)
async def get_model_mappings(
    ...
    try:
        return await service.get_model_mappings()
    except UpstreamAPIError as e:
        return JSONResponse(
            status_code=e.status_code,
            content={
...
@router.put(
    "/model-mappings",
    summary="批量更新模型映射表",
    description="批量替换 Kiro 前端模型名到真实模型名的映射关系（需要管理员权限）",
)
async def update_model_mappings(
    ...
    try:
        ...
        return await service.update_model_mappings(mappings)
    except UpstreamAPIError as e:
        return JSONResponse(
            status_code=e.status_code,
            content={
...
@router.post(
    "/model-mappings",
    summary="添加或更新单个模型映射",
    description="添加或更新单个 Kiro 前端模型名到真实模型名的映射（需要管理员权限）",
)
async def add_model_mapping(
    ...
    try:
        frontend_model = request.get("frontend_model")
        kiro_model = request.get("kiro_model")
        if not frontend_model or not kiro_model:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="frontend_model 和 kiro_model 参数都是必需的",
            )
        return await service.add_model_mapping(frontend_model, kiro_model)
    except UpstreamAPIError as e:
        return JSONResponse(
            status_code=e.status_code,
            content={
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"添加模型映射失败: {str(e)}",
        )


@router.delete(
    "/model-mappings/{frontend_model}",
    summary="删除单个模型映射",
    description="删除指定的 Kiro 模型映射（需要管理员权限）",
)
async def delete_model_mapping(
    ...
    try:
        return await service.delete_model_mapping(frontend_model)
    except UpstreamAPIError as e:
        return JSONResponse(
            status_code=e.status_code,
            content={
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"删除模型映射失败: {str(e)}",
        )


@router.post(
    "/model-mappings/reset",
    summary="重置模型映射为默认值",
    description="将 Kiro 模型映射重置为默认配置（需要管理员权限）",
)
async def reset_model_mappings(
    _: User = Depends(get_current_admin_user),
    service: KiroService = Depends(get_kiro_service),
):
    try:
        return await service.reset_model_mappings()
    except UpstreamAPIError as e:
        return JSONResponse(
            status_code=e.status_code,
            content={
